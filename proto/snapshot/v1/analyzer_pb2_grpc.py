...
class AnalyzerServiceServicer(object):
    ...
    def Analyze(self, request, context):
        """Analyze は静止画を受け取り解析結果を返します
        RPCメタデータ:
        Client -> Server
        - `request_id`: 要求のトレースに用いられる文字列
        - `device_id`: 画像が撮影されたデバイスの識別子
        - `context`: AnalyzeResponse.UpdateContextで設定されたstructをJSON形式にしたものがメタデータのvalueとして設定されます。
        Server -> Client
...
def add_AnalyzerServiceServicer_to_server(servicer, server):
    ...
    generic_handler = grpc.method_handlers_generic_handler(
            'safie.analyzer.snapshot.v1.AnalyzerService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))
    server.add_registered_method_handlers('safie.analyzer.snapshot.v1.AnalyzerService', rpc_method_handlers)
